...
class Count:
        ...
        def __init__(self):
                ...
                for line in infile: # creates list of stop_words
                        line = line.strip("\n")
        ...
        def get_num_words(self):
                unique_words = len(self.word_counts) # checks amount of keys
                return unique_words
        ...
        def increment_count(self,word):
                word = word.strip(string.punctuation).lower() # sanitizes input
                
                if word != "" and word not in self.stop_words:
                        if word in self.word_counts:
                                self.word_counts[word] += 1 # increments dictionary
                        else:
                                self.word_counts[word] = 1 # initializes term
                return 
...
def main():
        ...
        for line in infile:
                line = line.strip("\n") # strips the lines of \n
                words = line.split(" ") # creates list of words
                for i in range(len(words)):
                        counter.increment_count(words[i]) # counts words
        ...
        print("Unique words:", counter.get_num_words() - 1)
